...
//                        altNode.getElementsByTagName(XML_DESCRIPTION).item(0).getTextContent()));
    ...
    public static IndividualAttributeMaps readUserAttributeFile(String filename) {
        File xmlFile = new File(filename);
        if (!xmlFile.exists() || xmlFile.isDirectory())
            return null;
        
        IndividualAttributeMaps iam = new IndividualAttributeMaps(filename);
        ...
        try {
          ...
          iam.attributeColorMap = colorMap;
          ...
          for (int i = 0; i < crits.getLength(); i++) {
              Node node = crits.item(i);
              if (node.getNodeType() == Node.ELEMENT_NODE) {
                  parseCriteriaUser((Element) node, iam);
              }
          }
...
//          UserDialog.showError("Error in line " + ex.getMessage(), "File Error", vc.getFrame());
    ...
    private static void parseCriteriaUser(Element attrElem, IndividualAttributeMaps iam) 
                                                                throws IOException {
        try {
            double hr=1.0;
            if (attrElem.getAttribute("type").equals("abstract")) {
                ...
                hr = Double.parseDouble(attrElem.getAttribute("weight"));               
                Element domainElem = (Element) attrElem.getElementsByTagName(XML_DOMAIN).item(0);
                AttributeDomain domain = parseDomain(domainElem);
                if (domainElem.getAttribute("type").equals("continuous")) {
                    String unitsName = domainElem.getAttribute("unit");
