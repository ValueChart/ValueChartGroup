...
public class AttributePrimitiveData implements AttributeData {
    ...
    public String getOutputLogXML(boolean isRoot) {
        ...
        str += "<Primary name=\"" + getName() 
                + "\" weight=\"" + getWeight() 
                + "\" color=\"" + getName() 
                + "\" units=\"" + (t == AttributeDomain.DISCRETE ? "" : getUnitsName()) + "\">\n";
        str += "<Domain type=\"" + (t == AttributeDomain.DISCRETE ? "discrete" : "continuous") + "\">\n";

        double vals[] = d.getWeights();
        for (int i = 0; i < vals.length; i++) {
            str += "<DomainValue ";
            if (t == AttributeDomain.DISCRETE) {
                String elts[] = d.getElements();
                str += "key=\"" + elts[i] + "\" ";
            } else {
                double kts[] = d.getKnots();
    ...
    public String getOutputWeightXML(boolean isRoot) {
        ...
        str += "<Primary name=\"" + getName() 
                + "\" weight=\"" + getWeight() 
                + "\" color=\"" + getName() 
                + "\" units=\"" + (getDomain().getType() == AttributeDomain.DISCRETE ? "" : getUnitsName()) + "\"/>\n";
        return str;
    }
/*
    @Override
