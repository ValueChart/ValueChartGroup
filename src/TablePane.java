...
public class TablePane extends JLayeredPane {
    ...
    public AttributeCellGroup getAttributeCellGroup(String name) {
        AttributeCellGroup grpcell;
//        System.out.println("rowListGroup size:"+rowListGroup.size());
        for (Iterator it = rowListGroup.iterator(); it.hasNext();) {
            BaseTableContainerGroup comp = (BaseTableContainerGroup) it.next();
            if (comp.getTable() instanceof TablePane) {
            	grpcell = ((TablePane) comp.getTable()).getAttributeCellGroup(name);
                if (grpcell != null) {
                    return grpcell;
                }
            } else {
                if (comp.getName().equals(name)) {
                    return (AttributeCellGroup) comp.getTable();
                }
            }
        }
        return null;
    ...
    public void addRowGroup(BaseTableContainerGroup comp) {
        super.add(comp);
        rowListGroup.add(comp);
    }
...
/*        if(assignedHeight > 1.0){
        	for (Iterator it = rowList.iterator(); it.hasNext();) {
        		BaseTableContainer comp = (BaseTableContainer) it.next();
        		hr = comp.getHeightRatio();
        		comp.setHeightRatio(hr/assignedHeight);
        	}
        }*/
        int ypos = 0;
