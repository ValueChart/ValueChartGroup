/*
This class is for constructing the model instead of using a .vc file  
*/
import java.awt.*;
...
public class ConstructionView extends JPanel implements ChangeListener, ActionListener{
	...
	ArrayList<String> list; //list of files
    ...
    public void showChart(boolean fromCon){
    	if (fromCon)
    		if (chart != null)
        		chart.closeChart();
    	chart = new ValueChart(this,  filename, list, ValueChart.DEFAULT_DISPLAY, ValueChart.DEFAULT_COL_WIDTH, true, false,ValueChart.COLORFORUSER);  
    ...
    public void showConstructionView() {    	
        ...
        frame = new JDialog(ValueChartsPlus.frame, "ValueCharts - Model Construction");
        frame.setModal(true);
    ...
    public void createDataFile(String fname){      	
    	...
    	for (int i=0; i<pnlObjectives.prim_obj.size(); i++){
    		...
    		if (obj.color != Color.WHITE){
    			...
    			(float)obj.color.getGreen()/255 + " " + (float)obj.color.getBlue()/255 + "\n";
    		}
    		//if not, assign one
    		else{ 
    		...
    		while(st.nextToken() != StreamTokenizer.TT_EOF) {
    			...
    			if(st.lineno()==1){
     				...
     				if(st.ttype==StreamTokenizer.TT_WORD){
     					//if = unit set flag to true then have anotherit that processes i flag is true then set flag to false
